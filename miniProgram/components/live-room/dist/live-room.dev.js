...
var customFunction = {
  listenCallBack: function listenCallBack() {
    ...
    zego.onGetAnchorInfo = function (anchor_userid, anchro_username) {
      _this.onGetAnchorInfo(anchor_userid, anchro_username);
    };
    ...
    zego.onRecvJoinLiveRequest = function (requestId, from_userid, from_username, roomid) {
      _this.triggerEvent('RoomEvent', {
    ...
    zego.onRecvEndJoinLiveCommand = function (requestId, from_userid, from_username, roomid) {
      console.log('onRecvEndJoinLiveCommand', requestId, from_userid, from_username, roomid);
      zego.stopPublishingStream(_this.data.pushstreamid);
...
var liveRoomHandler = {
  ...
  start: function start(zegotoken, authorToken) {
    ...
    if (this.checkParam()) {
      var loginSucCallback = function loginSucCallback(streamList) {
        ...
        _this2.data.isCaster && zego.setPreferPublishSourceType(1);
        _this2.data.isCaster && _this2.doPublish();

        if (!_this2.data.isCaster && streamList.length == 0) {
          // console.error('主播已退出')
          //通知使用组件页面
          _this2.triggerEvent('RoomEvent', {
            tag: 'onRoomNotExist',
            code: 0,
            detail: {}
          });
      ...
      if (authorToken) {
        zego.loginWithAuthor(this.data.roomID, this.data.isCaster ? 1 : 2, zegotoken, authorToken, loginSucCallback, loginFailCallback);
      } else {
  ...
  respondJoinLive: function respondJoinLive(requestId, isgree) {
    ...
    zego.respondJoinLive(requestId, isgree, function (seq) {
  ...
  requestJionPusher: function requestJionPusher() {
    var _this6 = this;
    ...
    zego.requestJoinLive(this.data.mainPusherInfo.puserID, function (seq) {
  ...
  kickoutSubPusher: function kickoutSubPusher(ev) {
    zego.endJoinLive(ev.target.dataset.userid, function (seq) {
...
var sdkCallBackHandler = {
  onStreamUrlUpdate: function onStreamUrlUpdate(streamId, url, type) {
    ...
    if (type === 0) {
      ...
      var _streams = this.data.streamList.map(function (item) {
        ...
        if (item.userID == _this7.data.mainPusherInfo.puserID) {
          isHost = true;
        }
      ...
      var streamList = _streams.filter(function (item) {
        if (item.userID != _this7.data.mainPusherInfo.puserID) {
          return true;
        } else {
      ...
      if (this.data.isCaster) {
        ...
        self.setData({
          linkPusherInfo: _objectSpread({}, this.data.linkPusherInfo, {}, {
            stream_id: streamId,
            url: url,
            mode: 'RTC',
            minBitrate: 150,
  ...
  onStreamUpdated: function onStreamUpdated(type, streamList) {
    ...
    if (type === 0) {
      ...
      // delete
      var _streams = _toConsumableArray(this.data.streamList); //关闭子主播
      ...
      if (streamList.some(function (playItem) {
        return playItem.stream_id === _this8.data.mainPusherInfo.stream_id;
      })) {
        playingList = playingList.filter(function (playing) {
          return playing.stream_id !== _this8.data.mainPusherInfo.stream_id;
        });
        zego.stopPlayingStream(this.data.mainPusherInfo.stream_id); //通知使用组件页面
  ...
  onGetAnchorInfo: function onGetAnchorInfo(anchor_userid, anchro_username) {
...
Component(_objectSpread({}, _data_propoerty, {
   ...
   * 组件的方法列表
   */
  methods: _objectSpread({}, liveRoomHandler, {}, _liveRoomHandler, {}, sdkCallBackHandler, {}, mediaCallBackHandlers, {}, customFunction)
}));