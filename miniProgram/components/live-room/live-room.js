const zegoSdk = require('../../js/jZego-wx-1.1.1.js');
let zego;
let playingList = [];
...
let _data_propoerty = {
    ...
    properties: {
        ...
        debug: {type: Boolean, value: false,observer: function (newVal, oldVal) {
        ...
        if (this.checkParam()) {
            ...
            if (authorToken) {
                zego.loginWithAuthor(this.data.roomID, this.data.isCaster ? 1 : 2,
                    this.data.zegotoken, authorToken, loginSucCallback, loginFailCallback);
            } else {
                zego.login(this.data.roomID, this.data.isCaster ? 1 : 2, this.data.zegotoken, loginSucCallback, loginFailCallback)
            }
...
let _liveRoomHandler = {
    // 必填参数校验
    checkParam() {
        if (!this.data.zegotoken) {
            console.error('token input empty');
            return false;
        } else if (!zego) {
            ...
            if (!isHost) {
                ...
                if (this.data.mainPusherInfo.url && this.data.mainPusherInfo.mode != 'RTC') {
                    this.setData({
                        ...
                        //改变拉流mode 停止再次播放
                        const mainLive = wx.createLivePlayerContext(this.data.mainPusherInfo.stream_id, this);
                        mainLive.stop();
                        mainLive.play();
