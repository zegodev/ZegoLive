...
let {getLoginToken2} = require("../../../utils/server.js");
let zg;
//获取应用实例
const app = getApp();
let {appID, appSign, wsServerURL2, logServerURL2, tokenURL2,testEnvironment} = app.globalData;
...
Page({
    ...
    onLoad({roomID, roomName, loginType}) {
        console.log('>>>[live-room] onLoad, options are: ', roomID);
    ...
    onReady() {
        ...
        zg.config({
            ...
            server: wsServerURL2,        // 必填，服务器地址，由即构提供
            logUrl: logServerURL2,   // 必填，log 服务器地址，由即构提供
            audienceCreateRoom: false,     // 观众不允许创建房间
            testEnvironment:!!testEnvironment
        });
        this.bindCallBack();  //监听zego-sdk回调

        console.log('>>>[live-room] publishStreamID is: ' + this.data.publishStreamID);

        console.log(appID, appSign)
        // 进入房间，自动登录
        getLoginToken2(appID, appSign, this.data.userID).then(token => {
    ...
    bindCallBack() {
        ...
        zg.onStreamUrlUpdate = function (streamid, url, type, reason) {
            console.log(">>>[live-room] zg onStreamUrlUpdate, streamId: " + streamid + ', type: ' + (type === 0 ? 'play' : 'publish') + ', url: ' + url);
        ...
        zg.onDisconnect = function (err) {
            console.log(">>>[live-room] zg onDisconnect");
        };

        // 服务端主动推过来的 用户被踢掉在线状态事件
        zg.onKickOut = function (err) {
            console.log(">>>[live-room] zg onKickOut");
        };

        // 接收服务端主推送的自定义信令
        zg.onRecvCustomCommand = function (from_userid, from_idName, custom_content) {
            console.log(">>>[live-room] zg onRecvCustomCommand" + "from_userid: " + from_userid + "from_idName: " + from_idName + "content: ");
        ...
        zg.onStreamUpdated = function (updatedType, streamList) {
            console.log(">>>[live-room] zg onStreamUpdated, updatedType: " + (updatedType === 0 ? 'added' : 'deleted') + ', streamList: ');
        ...
        zg.onStreamExtraInfoUpdated = function (streamList) {
            console.log(">>>[live-room] zg onStreamExtraInfoUpdated");
        };

        // 服务端主动推过来的 流的播放状态, 视频播放状态通知
        // type: { start:0, stop:1};
        zg.onPlayStateUpdate = function (updatedType, streamID) {
            console.log(">>>[live-room] zg onPlayStateUpdate, " + (updatedType === 0 ? 'start ' : 'stop ') + streamID);
        ...
        zg.onRecvRoomMsg = function (chat_data, server_msg_id, ret_msg_id) {
            console.log(">>>[live-room] zg onRecvRoomMsg, data: ", chat_data);
        ...
        zg.onPlayQualityUpdate = function (streamID, streamQuality) {
            console.log(">>>[live-room] zg onPlayQualityUpdate", streamQuality);
        };

        // 推流后，服务器主动推过来的，流状态更新
        // type: { start: 0, stop: 1 }，主动停止推流没有回调，其他情况均回调
        zg.onPublishStateUpdate = function (type, streamid, error) {
            console.log('>>>[live-room] zg onPublishStateUpdate, streamid: ' + streamid + ', type: ' + (type === 0 ? 'start' : 'stop') + ', error: ' + error);
        ...
        zg.onGetAnchorInfo = function (anchorId, anchorName) {
            console.log('>>>[live-room] onGetAnchorInfo, anchorId: ' + anchorId + ', anchorName: ' + anchorName);
        ...
        zg.onRecvJoinLiveRequest = function (requestId, fromUserId, fromUsername, roomId) {
            console.log('>>>[live-room] onRecvJoinLiveRequest, roomId: ' + roomId + 'requestUserId: ' + fromUserId + ', requestUsername: ' + fromUsername);
            ...
            wx.showModal({
                ...
                success(res) {
                    if (res.confirm) {
                        console.log('>>>[live-room] onRecvJoinLiveRequest accept join live');
                        ...
                        console.log('>>>[live-room] onRecvJoinLiveRequest refuse join live');
                        zg.respondJoinLive(requestId, false); // true：同意；false：拒绝
        ...
        zg.onRecvEndJoinLiveCommand = function (requestId, fromUserId, fromUsername, roomId) {
            console.log('>>>[live-room] onRecvEndJoinLiveCommand, roomId: ' + roomId + 'requestUserId: ' + fromUserId + ', requestUsername: ' + fromUsername);
        };

        zg.onUserStateUpdate = function (roomId, userList) {
            console.log('>>>[live-room] onUserStateUpdate, roomID: ' + roomId + ', userList: ');
        ...
        zg.onUpdateOnlineCount = function (roomId, userCount) {
            console.log('>>>[live-room] onUpdateOnlineCount, roomId: ' + roomId + ', userCount: ' + userCount);
        };
    },

    setPlayUrl(streamid, url) {
        console.log('>>>[live-room] setPlayUrl: ', url);
        let self = this;
        if (!url) {
            console.log('>>>[live-room] setPlayUrl, url is null');
            return;
        }

        for (let i = 0; i < self.data.playStreamList.length; i++) {
            if (self.data.playStreamList[i]['streamID'] === streamid && self.data.playStreamList[i]['playUrl'] === url) {
                console.log('>>>[live-room] setPlayUrl, streamid and url are repeated');
                return;
            }
        }
    ...
    setPushUrl(url) {
        console.log('>>>[live-room] setPushUrl: ', url);
        let self = this;

        if (!url) {
            console.log('>>>[live-room] setPushUrl, url is null');
            return;
        }
    ...
    loginRoom(token) {
        let self = this;
        console.log('>>>[live-room] login room, roomID: ' + self.data.roomID, ', userID: ' + self.data.userID + ', userName: ' + self.data.userName);

        zg.login(self.data.roomID, self.data.loginType === "anchor" ? 1 : 2, token, function (streamList) {
            console.log('>>>[live-room] login success, streamList is: ');
            console.log(streamList);
            ...
            if (self.data.loginType === 'anchor') {
                console.log('>>>[live-room] anchor startPublishingStream, publishStreamID: ' + self.data.publishStreamID);
            ...
            console.log('>>>[live-room] login failed, error is: ', err);
            if (err) {
    ...
    startPlayingStreamList(streamList) {
        ...
        if (streamList.length === 0) {
            console.log('>>>[live-room] startPlayingStream, streamList is null');
            return;
        }
        ...
        console.log('>>>[live-room] startPlayingStream, preferPlaySourceType: ', self.data.preferPlaySourceType);

        // 获取每个 streamID 对应的拉流 url
        for (let i = 0; i < streamList.length; i++) {
            let streamID = streamList[i].stream_id;
            let anchorID = streamList[i].anchor_id_name;  // 推这条流的用户id
            console.log('>>>[live-room] startPlayingStream, playStreamID: ' + streamID + ', pushed by : ' + anchorID);
    ...
    stopPlayingStreamList(streamList) {
        ...
        if (streamList.length === 0) {
            console.log('>>>[live-room] stopPlayingStream, streamList is empty');
            return;
        }
        ...
        for (let i = 0; i < streamList.length; i++) {
            ...
            console.log('>>>[live-room] stopPlayingStream, playStreamID: ' + streamID);
            zg.stopPlayingStream(streamID);

            // 删除播放流列表中，删除的流
            for (let j = 0; j < playStreamList.length; j++) {
                if (playStreamList[j]['streamID'] === streamID) {
                    console.log('>>>[live-room] stopPlayingStream, stream to be deleted: ');
    ...
    //live-player 绑定拉流事件
    onPlayStateChange(e) {
        console.log('>>>[live-room] onPlayStateChange, code: ' + e.detail.code + ', message:' + e.detail.message);
    ...
    // live-pusher 绑定推流事件
    onPushStateChange(e) {
        console.log('>>>[live-room] onPushStateChange, code: ' + e.detail.code + ', message:' + e.detail.message);
    ...
    // 主播邀请连麦
    inviteJoinLive() {
        console.log('>>>[live-room] inviteJoinLive');

        zg.inviteJoinLive('', function (res) {
            console.log('>>>[live-room] inviteJoinLive sent succeeded');
        }, function (error) {
            console.log('>>>[live-room] inviteJoinLive sent failed, error: ', error);
        }, function (result, userID, userName) {
            console.log('>>>[live-room] inviteJoinLive, result: ' + result);
        });
    },
    ...
    requestJoinLive() {
        ...
        if (self.data.beginToPublish === true) {
            console.log('>>>[live-room] begin to publishing, ignore');
            return;
        }

        // 房间流已达上限，不处理
        if (self.data.reachStreamLimit === true) {
            console.log('>>>[live-room] reach stream limit, ignore');
            return;
        }
        ...
        if (self.data.isPublishing) {
            zg.endJoinLive(self.data.anchorID, function (result, userID, userName) {
                console.log('>>>[live-room] endJoinLive, result: ' + result);
            }, null);
        ...
        // 观众未连麦，点击开始推流
        console.log('>>>[live-room] audience requestJoinLive');
        self.setData({
            beginToPublish: true,
        });

        zg.requestJoinLive(self.data.anchorID, function (res) {
            console.log('>>>[live-room] requestJoinLive sent succeeded');
        }, function (error) {
            console.log('>>>[live-room] requestJoinLive sent failed, error: ', error);
        }, function (result, userID, userName) {
            console.log('>>>[live-room] requestJoinLive, result: ' + result);
            ...
            if (result === false) {
                ...
                // 主播同意连麦后，观众开始推流
                console.log('>>>[live-room] startPublishingStream, userID: ' + userID + ', publishStreamID: ' + self.data.publishStreamID);
                zg.setPreferPublishSourceType(self.data.preferPublishSourceType);
                zg.startPublishingStream(self.data.publishStreamID, '');
                      ...
                      if (data.code == 200) {
                        ...
                        zg.sendRoomMsg(2, 2, msg,
                            function (seq, msgId, msg_category, msg_type, msg_content) {
                                console.log('>>>[live-room] onComment success');
                            }, function (err, seq, msg_category, msg_type, msg_content) {
                                console.log('>>>[live-room] onComment, error: ');
                                console.log(err);
                            });
                      } else {
                          wx.showToast({
    ...
    onComment() {
        console.log('>>>[live-room] begin to comment', this.data.inputMessage);
        ...
        console.log('>>>[live-room] currentMessage', this.data.inputMessage);
    ...
    showMessage() {
        console.log('>>>[live-room] onShowMessage');
    ...
    onShow() {
        ...
        if (this.data.isPublishing) {
            console.log('>>>[live-room] sendRoomMsg, begin to send');

            let roomData = "onShow." + this.data.publishStreamID;
            zg.sendRoomMsg(2, 1, roomData,
                function (seq, msgId, msg_category, msg_type, msg_content) {
                    console.log('>>>[live-room] sendRoomMsg, send succeeded');
                },
                function (err, seq, msg_category, msg_type, msg_content) {
                    console.log('>>>[live-room] sendRoomMsg, send failed, err: ', err);
                }
            );
        }
        ...
        wsServerURL2 = getApp().globalData.wsServerURL2;
        logServerURL2 = getApp().globalData.logServerURL2;
        tokenURL2 = getApp().globalData.tokenURL2;
     ...
     */
    onHide() {
        console.log('>>>[live-room] onHide');

        // 退后台停止拉流
        for (let i = 0; i < this.data.playStreamList.length; i++) {
            console.log('>>>[live-room] onHide stopPlayStream: ', this.data.playStreamList[i]['streamID']);
     ...
     */
    onUnload() {
        console.log('>>>[live-room] onUnload');

        // 停止拉流
        let streamList = this.data.playStreamList;
        for (let i = 0; i < streamList.length; i++) {
            let streamID = streamList[i]['streamID'];
            console.log('>>>[live-room] onUnload stop play stream, streamid: ' + streamID);
            zg.stopPlayingStream(streamID);
        ...
        // 停止推流
        if (this.data.isPublishing) {
            console.log('>>>[live-room] stop publish stream, streamid: ' + this.data.publishStreamID);
