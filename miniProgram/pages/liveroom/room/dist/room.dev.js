...
var _require = require("../../../js/jZego-wx-1.5.0.js"),
    ZegoClient = _require.ZegoClient; // let { ZegoClient } = require("miniprogram-zego");


var _require2 = require("../../../utils/util.js"),
    format = _require2.format,
    sharePage = _require2.sharePage;

var _require3 = require("../../../utils/server.js"),
    getLoginToken = _require3.getLoginToken;

var zg; //获取应用实例

var app = getApp();
var _app$globalData = app.globalData,
    appID = _app$globalData.liveAppID,
    wsServerURL = _app$globalData.wsServerURL,
    logServerURL = _app$globalData.logServerURL,
    tokenURL = _app$globalData.tokenURL;
var recorderManager;
/**
 * 页面的初始数据
 */
...
Page({
  data: {
    ...
    // 推流 ID
    pusherVideoContext: {},
    // live-pusher Context，内部只有一个对象
    playStreamList: [],
    // 拉流流信息列表，列表中每个对象结构为 {anchorID:'xxx', streamID:'xxx', playContext:{}, playUrl:'xxx', playingState:'xxx'}
    ...
    // 0：auto；1：从 bgp 拉流
    upperStreamLimit: 8,
    // 房间内限制为最多 4 条流，当流数大于 4 条时，禁止新进入的用户连麦
    connectType: -1,
    // -1为初始状态，1为连接，0断开连接
    tapTime: "",
    pushUrl: "",
    ...
    // 请求连麦的成员列表
    messageList: [],
    // 消息列表，列表中每个对象结构为 {name:'xxx', time:xxx, content:'xxx'}
    isCommentShow: false,
    inputMessage: "",
    isMessageHide: true,
    scrollToView: "",
  ...
  onReady: function onReady() {
    ...
    zg.config({
      ...
      // 必填，应用id，由即构提供
      idName: this.data.userID,
      // 必填，用户自定义id，全局唯一
      nickName: this.data.userName,
      // 必填，用户自定义昵称
      remoteLogLevel: 2,
      // 日志上传级别，建议取值不小于 logLevel
      logLevel: 0,
      // 日志级别，debug: 0, info: 1, warn: 2, error: 3, report: 99, disable: 100（数字越大，日志越少）
      server: wsServerURL,
      // 必填，服务器地址，由即构提供
      logUrl: logServerURL,
      // 必填，log 服务器地址，由即构提供
    ...
    console.log('>>>[liveroom-room] publishStreamID is: ' + this.data.publishStreamID); // 进入房间，自动登录

    getLoginToken(this.data.userID, appID).then(function (token) {
      _this.setData({
        token: token
      });
  ...
  bindCallBack: function bindCallBack() {
    var self = this; // startPlayStream、startPublishStream 后，服务端主动推过来的流更新事件
    ...
    zg.onPlayStateUpdate = function (updatedType, streamID) {
      ...
      if (updatedType === 1) {
        ...
        // 流播放成功, 更新状态
        for (var _i = 0; _i < self.data.playStreamList.length; _i++) {
          if (self.data.playStreamList[_i]['streamID'] === streamID) {
            self.data.playStreamList[_i]['playingState'] = 'succeeded';
          }
        }
      }
    ...
    zg.onRecvRoomMsg = function (chat_data, server_msg_id, ret_msg_id) {
      console.log(">>>[liveroom-room] zg onRecvRoomMsg, data: ", chat_data); // 收到其他成员的回到前台通知
    ...
    zg.onRecvJoinLiveRequest = function (requestId, fromUserId, fromUsername, roomId) {
      ...
      wx.showModal({
        ...
        success: function success(res) {
          if (res.confirm) {
            console.log('>>>[liveroom-room] onRecvJoinLiveRequest accept join live'); // self.switchPusherOrPlayerMode('pusher', 'RTC');
    ...
    zg.onUserStateUpdate = function (roomId, userList) {
      ...
      console.log(userList); //for (let i = 0; i < userList.length; i++) {
  ...
  setPlayUrl: function setPlayUrl(streamid, url) {
    ...
    var streamInfo = {};
    var isStreamRepeated = false; // 相同 streamid 的源已存在，更新 Url

    for (var _i2 = 0; _i2 < self.data.playStreamList.length; _i2++) {
      if (self.data.playStreamList[_i2]['streamID'] === streamid) {
        isStreamRepeated = true;
        self.data.playStreamList[_i2]['playUrl'] = url;
        self.data.playStreamList[_i2]['playingState'] = 'initial';
        break;
      }
    ...
    self.setData({
      ...
      if (isStreamRepeated) {
        self.data.playStreamList.forEach(function (streamInfo) {
  ...
  setPushUrl: function setPushUrl(url) {
    ...
    self.setData({
      ...
      self.data.pusherVideoContext.start(); // self.animation.rotate(720).step();
      // self.setData({animation: this.animation.export()});
    });
  },
  // 登录房间
  loginRoom: function loginRoom(token) {
    var self = this;
    ...
    zg.login(self.data.roomID, self.data.loginType === "anchor" ? 1 : 2, token, function (streamList) {
      ...
      self.startPlayingStreamList(streamList); // 主播登录成功即推流
  ...
  startPlayingStreamList: function startPlayingStreamList(streamList) {
    var self = this;
    ...
    console.log('>>>[liveroom-room] startPlayingStream, preferPlaySourceType: ', self.data.preferPlaySourceType); // 获取每个 streamID 对应的拉流 url
  ...
  stopPlayingStreamList: function stopPlayingStreamList(streamList) {
    var self = this;
    ...
    for (var i = 0; i < streamList.length; i++) {
      ...
      zg.stopPlayingStream(streamID); // 删除播放流列表中，删除的流
      ...
      for (var j = 0; j < playStreamList.length; j++) {
        if (playStreamList[j]['streamID'] === streamID) {
          ...
          playStreamList[j]['playContext'] && playStreamList[j]['playContext'].stop(); // let content = '一位观众结束连麦，停止拉流';
          // wx.showToast({
          //   title: content,
          //   icon: 'none',
          //   duration: 2000
    ...
    self.setData({
      ...
      if (self.data.loginType === 'anchor' && self.data.playStreamList.length === 0) {// self.switchPusherOrPlayerMode('pusher', 'live');
      }
    });
  },
  //live-player 绑定拉流事件
  onPlayStateChange: function onPlayStateChange(e) {
    ...
    if (e.detail.code === 2002 || e.detail.code === 2004) {
      ...
      // }
      this.data.playStreamList.forEach(function (item) {
        if (item['playContext']) {
  ...
  updatePlayingStateOnly: function updatePlayingStateOnly(e, newState) {
  ...
  onPushStateChange: function onPushStateChange(e) {
    console.log('>>>[liveroom-room] onPushStateChange, code: ' + e.detail.code + ', message:' + e.detail.message); // 透传推流事件给 SDK，type 1 推流
  ...
  // 主播邀请连麦
  inviteJoinLive: function inviteJoinLive() {
  ...
  // 观众请求连麦，或主播邀请连麦
  requestJoinLive: function requestJoinLive() {
    var self = this;
    if (this.data.loginType === 'anchor') return; // 防止两次点击操作间隔太快
    ...
    if (self.data.isPublishing) {
      ...
      self.setData({
        isPublishing: false,
        pushUrl: ""
      }, function () {// 自己停止推流，不会收到流删减消息，所以此处需要主动调整视图大小
      }); // setTimeout(() => {
      //     // 回前台重新拉流
    ...
    zg.requestJoinLive(self.data.anchorID, function (res) {
      ...
      console.log('>>>[liveroom-room] requestJoinLive, result: ' + result); // 待补充，校验 userID
  ...
  handleMultiJoinLive: function handleMultiJoinLive(requestJoinLiveList, requestId, self) {
  ...
  snapshot: function snapshot() {
    ...
    var sucCallback = function sucCallback(ret) {
      console.log('>>>[liveroom-room] snapshot success');
      console.log('ret', ret.tempImagePath);
      wx.showLoading({
        title: '正在上传...'
      });
      var imgPath = 'sdk-doc/mini-snapshot-' + new Date().getTime() + '.jpg'; // 上传图片
      ...
      wx.uploadFile({
        ...
        success: function success(res) {
          ...
          if (data.code == 200) {
            ...
            var msg = 'https://***' + imgPath; // 发送图片URL

            zg.sendRoomMsg(2, 2, msg, function (seq, msgId, msg_category, msg_type, msg_content) {
    ...
    if (this.data.loginType === 'anchor') {
      ...
      if (this.data.playStreamList[0].playContext.snapshot) {
        this.data.playStreamList.forEach(function (streamInfo) {
  ...
  _saveImageToPhotosAlbum: function _saveImageToPhotosAlbum(imgPath) {
    // 保存图片到本地相册
    wx.getSetting({
      success: function success(res) {
  ...
  enableMute: function enableMute() {
  ...
  playOrStopBgm: function playOrStopBgm() {
  ...
  onNetworkStatus: function onNetworkStatus() {
    ...
    wx.onNetworkStatusChange(function (res) {
      ...
      if (res.isConnected && _this2.data.connectType === 0 && zg) {
        console.log('connectType', _this2.data.connectType);
        zg.setUserStateUpdate(true);

        _this2.loginRoom(_this2.data.token);
      }
    });
  },
  ...
  onComment: function onComment() {
    ...
    this.showMessage();
    zg.sendRoomMsg(1, 1, message.content, function (seq, msgId, msg_category, msg_type, msg_content) {
      console.log('>>>[liveroom-room] onComment success');
    }, function (err, seq, msg_category, msg_type, msg_content) {
      console.log('>>>[liveroom-room] onComment, error: ');
      console.log(err);
  ...
  record: function record() {
    ...
    recorderManager.onStop(function (res) {
      ...
      _this3.uploadServer(tempFilePath); // wx.saveVideoToPhotosAlbum({
  ...
  uploadServer: function uploadServer(tempPath) {
    var filePath = 'sdk-doc/mini-record-' + new Date().getTime() + '.pcm'; // 上传文件
    ...
    wx.uploadFile({
      ...
      success: function success(res) {
        ...
        if (data.code == 200) {// wx.hideLoading()
          // wx.showToast({
          //     title: '发送成功',
          //     duration: 1000,
          //     mask: true
  ...
  onUnload: function onUnload() {
    console.log('>>>[liveroom-room] onUnload'); // 停止拉流
